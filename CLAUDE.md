...
| `aif-lsp` | LSP server — diagnostics, semantic tokens, symbol collection (tower-lsp) |
...
- `ViewMode` — Author, Llm, Api view modes for pre-emit AST filtering (in `aif-core::view`)
...
aif compile input.aif -f html|markdown|lml|lml-compact|lml-conservative|lml-moderate|lml-aggressive|json|binary-wire|binary-token|pdf [-o output] [--view author|llm|api]
...
aif skill resolve input.aif              # Resolve inheritance chain (extends)
...
`crates/aif-core/src/lint.rs` — 9 structural checks for any AIF document (not just skills): ClaimsWithoutEvidence, BrokenReferences, BrokenEvidenceLinks, OrphanedMedia, DuplicateIds, EmptySections (nested only), MissingMetadata, EmptyFootnotes, MalformedTables. `DocLintResult` includes optional `block_id` for precise error location. CLI: `aif lint doc.aif [--format text|json]`. `UndefinedTerms` check detects terms in `@claim` blocks not defined in any `@definition` block.
...
`crates/aif-core/src/view.rs` — Pre-emit AST filtering with `ViewMode` enum (Author, Llm, Api). `filter_for_view()` clones and strips blocks before any emitter sees them — zero emitter changes required. LLM view strips `@example`/`@scenario` blocks and collapses code blocks >20 lines to first 5 lines + truncation notice. API view keeps only `@tool`, `@output_contract`, `@precondition` blocks plus Definition/Requirement semantic blocks, with empty container pruning. CLI: `aif compile input.aif -f html --view llm`. 9 unit tests.

### Undefined Terms Lint
`UndefinedTerms` check in `aif-core::lint` — Extracts defined terms from `@definition` blocks (via bold/emphasis inlines or "is"/"means" separators), flags undefined terms used in `@claim` blocks. Zero false positives across all 12 example documents.

### Skill Inheritance
`crates/aif-skill/src/inherit.rs` — `@skill[extends="base-debugging"]` attribute enables skill inheritance with recursive resolution and cycle detection. Block-level merging: Steps matched by `order` attribute, other blocks by type+id. Child blocks override parent, unmatched parent blocks are inherited. CLI: `aif skill resolve child.aif`. Example pair: `examples/base-debugging.aif` + `examples/api-debugging.aif`. 7 tests including cycle detection and multi-level inheritance.

### Citation Precision Benchmark
`benchmarks/citation-precision/` — 3-file Python benchmark (`benchmark.py`, `questions.py`, `analysis.py`). Chunks documents with 4 strategies × 4 token budgets, queries LLM with ground-truth Q&A (12 questions across 3 documents), scores answer accuracy + citation precision/recall/F1. Requires `ANTHROPIC_API_KEY`.

### LSP Server
`crates/aif-lsp/` — tower-lsp MVP with 4 modules: `main.rs` (server lifecycle), `diagnostics.rs` (parse errors + lint with line/col positions), `semantic_tokens.rs` (8 token types from lexer), `symbols.rs` (block ID collection for go-to-definition stub). 9 tests passing.
