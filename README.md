...
See [`tutorial/skillforge_tutorial.ipynb`](tutorial/skillforge_tutorial.ipynb) for a full walkthrough of all 15 functions.

### AIF v2 syntax (v0.4.0)

v0.4.0 is **v2-only**: containers `@skill`, `@artifact_skill`, `@scenario` close with `@/skill` / `@/artifact_skill` / `@/scenario`. Every other block auto-closes at the next `@` directive — no `@end` tokens anywhere.

```aif
# v1 (rejected by parser)              # v2
...
Files with legacy `@end` are rejected with a migration hint. Convert old files:
...
Or from Python: `skillforge.migrate_syntax(src)`.
